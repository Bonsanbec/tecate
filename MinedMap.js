...
const MinedMapLayer = L.TileLayer.extend({
    ...
    getTileUrl: function (coords) {
        ...
        return `maps/data/${this.layer}/${z}/r.${coords.x}.${coords.y}.${this.ext}`;
    },
});
...
async function loadSigns(signLayer) {
    const response = await fetch('maps/data/entities.json', { cache: 'no-store' });
...
window.createMap = function () {
    (async function () {
        const response = await fetch('maps/data/info.json', { cache: 'no-store' });
        const res = await response.json();
